#!/usr/bin/env bash
set -euo pipefail

BASE="${BASE_URL:-${BASE:-https://beta.golfmeadows.org}}"
SESSION_COOKIE="${SESSION_COOKIE:-}"

TMP_BODY="$(mktemp)"
trap 'rm -f "$TMP_BODY"' EXIT

check_disabled_json() {
  local name="$1"
  local method="$2"
  local url="$3"
  local payload_type="${4:-NONE}"
  local data="${5:-}"
  local cookie="${6:-}"

  local curl_args=(
    -sS
    -o "$TMP_BODY"
    -w "%{http_code}"
    -X "$method"
    "$url"
  )

  if [[ "$payload_type" == "JSON" ]]; then
    curl_args+=(-H "Content-Type: application/json" --data "$data")
  elif [[ "$payload_type" == "FORM" ]]; then
    curl_args+=(-H "Content-Type: application/x-www-form-urlencoded" --data "$data")
  fi

  if [[ -n "$cookie" ]]; then
    curl_args+=(-H "Cookie: session=$cookie")
  fi

  local code
  code="$(curl "${curl_args[@]}")"

  if [[ "$code" != "403" ]]; then
    echo "FAIL [$name]: expected HTTP 403, got $code"
    echo "---- response body ----"
    cat "$TMP_BODY"
    echo
    echo "-----------------------"
    return 1
  fi

  if ! rg -q '"status":"disabled"' "$TMP_BODY"; then
    echo "FAIL [$name]: missing status=disabled"
    echo "---- response body ----"
    cat "$TMP_BODY"
    echo
    echo "-----------------------"
    return 1
  fi

  if ! rg -q '"error":"This feature is currently disabled by the administrator\."' "$TMP_BODY"; then
    echo "FAIL [$name]: missing expected error message"
    echo "---- response body ----"
    cat "$TMP_BODY"
    echo
    echo "-----------------------"
    return 1
  fi

  echo "PASS [$name]"
}

echo "Running disabled-feature checks on $BASE"
echo

check_disabled_json \
  "amenities-get-api" \
  "GET" \
  "$BASE/api/amenities/1/bookings"

check_disabled_json \
  "amenities-book-api" \
  "POST" \
  "$BASE/api/amenities/book" \
  "JSON" \
  '{"amenity_id":"1","resident_name":"QA Resident","resident_email":"qa@example.com","booking_date":"2030-01-10","start_time":"10:00","end_time":"11:00"}'

echo "INFO: /service-tickets may be a redirecting form endpoint in some deployments; checking separately."
ticket_code="$(curl -sS -o "$TMP_BODY" -w "%{http_code}" -X POST "$BASE/service-tickets" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  --data "full_name=QA+Resident&flat_number=A-101&email=qa%40example.com&category=Plumber&description=Blocked+test")"
if [[ "$ticket_code" == "403" ]]; then
  if ! rg -q '"status":"disabled"' "$TMP_BODY"; then
    echo "FAIL [ticket-create-public]: got 403 but missing status=disabled"
    cat "$TMP_BODY"
    exit 1
  fi
  echo "PASS [ticket-create-public]: 403 disabled JSON"
elif [[ "$ticket_code" == "302" ]]; then
  echo "WARN [ticket-create-public]: got 302 redirect (likely HTML form guard); API guards still enforced."
else
  echo "FAIL [ticket-create-public]: expected 403 or 302, got $ticket_code"
  cat "$TMP_BODY"
  exit 1
fi

if [[ -n "$SESSION_COOKIE" ]]; then
  echo "SESSION_COOKIE detected. Running admin checks..."
  check_disabled_json \
    "admin-ticket-update" \
    "POST" \
    "$BASE/admin/manage-tickets/1/update" \
    "FORM" \
    "status=Open&admin_notes=Blocked+test" \
    "$SESSION_COOKIE"

  check_disabled_json \
    "admin-amenity-pricing-update" \
    "POST" \
    "$BASE/admin/amenities/pricing" \
    "FORM" \
    "amenity_id=1&cost=0" \
    "$SESSION_COOKIE"

  check_disabled_json \
    "admin-directory-create" \
    "POST" \
    "$BASE/admin/directory-items" \
    "FORM" \
    "category=services_directory&title=Curl+Vendor&description=Blocked+test" \
    "$SESSION_COOKIE"
else
  echo "INFO: SESSION_COOKIE not set; skipping admin endpoint checks."
fi

echo
echo "PASS: All disabled-feature checks passed."
